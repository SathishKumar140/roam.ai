...
5. Present returned properties with their neighborhood, distance or transit highlights to nearby landmarks or stations (from `distance_highlights`), nightly/total prices, and exact public source URLs including `[📍 View on Google Maps](<google_maps_url>)` so travelers can inspect the location and plan their routes. Search results do not establish a booking or guaranteed availability.
