                    ...
                    tool_calls=[
                        {
                            "name": "task",
                            "id": "delegate",
                            "args": {"subagent_type": specialist, "description": "Handle the scoped request with your supplied tool."},
                        }
                    ],
                ),
...
async def test_roamai_companion_travel_proposal():
    ...
    result, model = await invoke_specialist(
        "travel_specialist", search_hotels, {"destination": "Bali", "adults": 2, "currency": "USD"}, context
    )
...
async def test_roamai_companion_vision_photo_analysis():
    ...
    result, model = await invoke_specialist(
        "vision_specialist", analyze_attached_image, {"question": "What is this place?"}, {"has_attached_image": False}
    )
...
async def test_roamai_companion_expense_logging_and_balance():
    ...
    result, model = await invoke_specialist(
        "expense_specialist", get_group_balances, {}, {"tasks": {"expenses": [{"amount": "120", "currency": "USD", "state": "pending"}]}}
    )
