...
class RoamAIStore:
    def __init__(self, path: str):
        ...
        with self.connect() as connection:
            ...
            connection.execute(
                "CREATE VIRTUAL TABLE IF NOT EXISTS roamai_topic_search USING fts5(topic_id UNINDEXED, group_id UNINDEXED, content)"
            )
            connection.execute("""
    ...
    def context(self, group_id: str, before_id: int = 9223372036854775807, selected_topic_id: str | None = None) -> dict:
        with self.connect() as connection:
            rows = connection.execute(
                ...
                (group_id, before_id, group_id, before_id),
            ).fetchall()
            selected = [selected_topic_id] if selected_topic_id else []
            if not selected_topic_id:
                text = " ".join(
                    ChannelEvent.model_validate_json(row["payload"]).text or "" for row in reversed(rows) if row["state"] == "processing"
                )
                ...
                selected.extend(
                    row[0]
                    for row in connection.execute(
                        "SELECT topic_id FROM roamai_outbox WHERE group_id = ? AND state = 'sent' AND topic_id IS NOT NULL ORDER BY created DESC LIMIT 4",
                        (group_id,),
                    )
                )
                selected.extend(
            ...
            topics = connection.execute(
                f"SELECT * FROM roamai_topics WHERE group_id = ? AND id IN ({placeholders}) ORDER BY updated DESC",
                (group_id, *selected),
            ).fetchall()
            preferences = connection.execute(
                f"SELECT topic_id, user_id, key, value, evidence_id FROM roamai_scoped_preferences WHERE group_id = ? AND (topic_id = '' OR topic_id IN ({placeholders}))",
                (group_id, *selected),
            ).fetchall()
            facts = connection.execute(
                f"SELECT * FROM roamai_topic_facts WHERE group_id = ? AND topic_id IN ({placeholders})", (group_id, *selected)
            ).fetchall()
        ...
        return {
            ...
            "topics": [
                ...
                | {
                    ...
                    "facts": {fact["key"]: dict(fact) for fact in facts if fact["topic_id"] == row["id"]},
                }
                for row in topics
            ],
            ...
            "outbound": [
                dict(row) | {"payload": json.loads(row["payload"]), "provider_ids": json.loads(row["provider_ids"])} for row in outbound
            ],
        }
    ...
    def save_topic(self, group_id: str, source_id: int, observation, participant_ids: list[str]) -> str:
        ...
        with self.connect() as connection:
            ...
            evidence_ids = {str(source_id), *observation.evidence_message_ids, *(fact.evidence_message_id for fact in observation.facts)}
            ...
            for fact in observation.facts:
                ...
                """,
                    (group_id, fact.evidence_message_id, source_id, topic_id),
                ).fetchone()
                if not source:
                    continue
            ...
            topic = connection.execute(
                "SELECT title, summary, initial_summary FROM roamai_topics WHERE id = ? AND group_id = ?", (topic_id, group_id)
            ).fetchone()
            facts = connection.execute(
                "SELECT key, value FROM roamai_topic_facts WHERE topic_id = ? AND group_id = ?", (topic_id, group_id)
            ).fetchall()
    ...
    def set_topic_state(self, group_id: str, topic_id: str, state: str):
        ...
        with self.connect() as connection:
            connection.execute(
                "UPDATE roamai_topics SET state = ?, updated = ? WHERE group_id = ? AND id = ?", (state, time.time(), group_id, topic_id)
            )
    ...
    def save_preference(
        ...
        with self.connect() as connection:
            source = connection.execute(
                "SELECT payload FROM roamai_inbox WHERE group_id = ? AND id = ?", (group_id, evidence_id)
            ).fetchone()
            if not source or not topic_id:
                return False
            incoming = ChannelEvent.model_validate_json(source["payload"])
            if (
                incoming.sender.id != user_id
                or not evidence_quote
                or evidence_quote not in (incoming.text or "")
                or value not in evidence_quote
            ):
                return False
    ...
    def complete(self, rows: list[dict], topic_id: str | None = None, response: dict | None = None, kind: str = "response"):
        ...
        with self.connect() as connection:
            ...
            newer = connection.execute(
                "SELECT 1 FROM roamai_inbox WHERE group_id = ? AND id > ? LIMIT 1", (group_id, rows[-1]["id"])
            ).fetchone()
            if response and not (kind == "offer" and newer):
                if kind == "response" and "awaiting_reply" not in response:
                    response = response | {
                        "awaiting_reply": bool(
                            response.get("addressed_to") and "?" in response.get("text", "") and not response.get("buttons")
                        )
                    }
    ...
    def pending_reply(self, group_id, user_id, before_id):
        with self.connect() as connection:
            ...
            """,
                (group_id, time.time() - 3600, before_id, user_id),
            ).fetchone()
            ...
            """,
                (group_id, row["source_id"], before_id, user_id),
            ).fetchone()
    ...
    def retry(self, rows: list[dict], error: str):
        ...
        with self.connect() as connection:
            for row in rows:
                connection.execute(
                    "UPDATE roamai_inbox SET state = 'pending', available = ?, error = ? WHERE id = ?",
                    (time.time() + 2**attempts + random.uniform(0, 1), error[:500], row["id"]),
                )
    ...
    def delivery_result(self, delivery: dict, success: bool, provider_ids=(), error="", retry_after: float | None = None, permanent=False):
        ...
        with self.connect() as connection:
            connection.execute(
                "UPDATE roamai_outbox SET state = ?, attempts = ?, available = ?, error = ?, provider_ids = ? WHERE id = ?",
                (state, attempts, now + delay, error[:500], json.dumps(list(provider_ids)), delivery["id"]),
            )
            if success and delivery["kind"] == "offer":
                connection.execute("UPDATE roamai_groups SET last_offer = ? WHERE id = ?", (now, delivery["group_id"]))
                connection.execute(
                    "UPDATE roamai_topics SET state = 'offered', last_offer = ?, offered_to = ? WHERE id = ? AND group_id = ?",
                    (now, delivery["payload"].get("offered_to"), delivery["topic_id"], delivery["group_id"]),
                )
    ...
    def create_poll(self, group_id, topic_id, question, options, minutes, request_id):
        if (
            ...
            or any(not option.strip() or len(option) > 20 for option in options)
        ):
        ...
        with self.connect() as connection:
            ...
            connection.execute(
                "INSERT OR IGNORE INTO roamai_polls VALUES (?, ?, ?, ?, ?, ?, 'open', ?)",
                (poll_id, group_id, topic_id, question, json.dumps(options), time.time() + minutes * 60, f"{group_id}:{request_id}"),
            )
        return poll_id
    ...
    def open_polls(self, group_id, topic_id):
        with self.connect() as connection:
            rows = connection.execute(
                "SELECT * FROM roamai_polls WHERE group_id = ? AND topic_id = ? AND state = 'open' AND closes > ?",
                (group_id, topic_id, time.time()),
            ).fetchall()
    ...
    def vote(self, group_id, poll_id, user_id, choice):
        with self.connect() as connection:
            ...
            poll = connection.execute(
                "SELECT * FROM roamai_polls WHERE id = ? AND group_id = ? AND state = 'open' AND closes > ?",
                (poll_id, group_id, time.time()),
            ).fetchone()
            if not poll or not 0 <= choice < len(json.loads(poll["options"])):
                return False
            connection.execute(
                "INSERT INTO roamai_votes VALUES (?, ?, ?) ON CONFLICT(poll_id, user_id) DO UPDATE SET choice = excluded.choice",
        ...
        with self.connect() as connection:
            ...
            for poll in connection.execute("SELECT * FROM roamai_polls WHERE state = 'open' AND closes <= ?", (now,)).fetchall():
                ...
                counts = {
                    ...
                    for row in connection.execute(
                        "SELECT choice, COUNT(*) AS total FROM roamai_votes WHERE poll_id = ? GROUP BY choice", (poll["id"],)
                    )
                }
                lines = ["Poll closed: " + poll["question"]] + [
                    f"{option}: {counts.get(index, 0)} vote(s)" for index, option in enumerate(options)
                ]
                if not counts:
        ...
        with self.connect() as connection:
            return bool(
                connection.execute(
                    ...
                    (
                        "active" if accepted else "declined",
                        time.time(),
                        topic_id,
                        group_id,
                        "active" if accepted else "declined",
    ...
    def propose_expense(self, group_id, topic_id, payer_id, amount, currency, description, participant_ids, request_id):
        ...
        with self.connect() as connection:
            ...
            existing = connection.execute(
                "SELECT * FROM roamai_expenses WHERE group_id = ? AND topic_id = ?", (group_id, topic_id)
            ).fetchone()
            if existing and (existing["id"] != expense_id or existing["state"] != "pending"):
                raise ValueError(
                    "This topic already has an expense. Do not recreate or reopen it; a distinct payment needs a new topic and new consent."
                )
            if existing and (
                Decimal(existing["amount"]) != number
                or existing["currency"] != currency
                or existing["payer_id"] != payer_id
                or json.loads(existing["participants"]) != participants
            ):
    ...
    def confirm_expense(self, group_id, expense_id, user_id, accepted=True):
        with self.connect() as connection:
            ...
            connection.execute(
                "UPDATE roamai_expenses SET confirmed = ?, state = ? WHERE id = ?", (json.dumps(sorted(confirmed)), state, expense_id)
            )
            return state
    ...
    def tasks(self, group_id, topic_id):
        with self.connect() as connection:
            ...
            reminders = connection.execute(
                "SELECT * FROM roamai_reminders WHERE group_id = ? AND topic_id = ?", (group_id, topic_id)
            ).fetchall()
        return {
            "polls": self.open_polls(group_id, topic_id),
    ...
    def balances(self, group_id):
        ...
        for row in rows:
            ...
            precision = (
                0 if row["currency"] in {"JPY", "KRW", "VND"} else 3 if row["currency"] in {"BHD", "KWD", "OMR", "JOD", "TND"} else 2
            )
    ...
    def schedule_reminder(self, group_id, topic_id, text, due, request_id):
        ...
        with self.connect() as connection:
            connection.execute(
                "INSERT OR IGNORE INTO roamai_reminders VALUES (?, ?, ?, ?, ?, 'pending')", (reminder_id, group_id, topic_id, text, due)
            )
        return reminder_id
    ...
    def cancel_reminder(self, group_id, topic_id, reminder_id):
        with self.connect() as connection:
            return bool(
                connection.execute(
                    "UPDATE roamai_reminders SET state = 'cancelled' WHERE id = ? AND group_id = ? AND topic_id = ? AND state = 'pending'",
                    (reminder_id, group_id, topic_id),
                ).rowcount
            )
    ...
    def release_due_reminders(self):
        ...
        with self.connect() as connection:
            ...
            for row in connection.execute("SELECT * FROM roamai_reminders WHERE state = 'pending' AND due <= ?", (now,)).fetchall():
                topic = connection.execute(
                    "SELECT state FROM roamai_topics WHERE id = ? AND group_id = ?", (row["topic_id"], row["group_id"])
                ).fetchone()
                valid = topic and topic["state"] == "active" and now - row["due"] < 3600
                if valid:
                    self._insert_outbox(
                        connection, "reminder:" + row["id"], row["group_id"], row["topic_id"], {"text": row["text"]}, "reminder", None, now
                    )
    ...
    def web_messages(self, group_id, after=0):
        with self.connect() as connection:
            rows = connection.execute(
                "SELECT rowid AS sequence, payload FROM roamai_outbox WHERE group_id = ? AND state = 'sent' AND rowid > ? ORDER BY rowid LIMIT 100",
                (group_id, after),
            ).fetchall()
    ...
    def queue_status(self):
        ...
        with self.connect() as connection:
            inbox = {
                row["state"]: row["count"] for row in connection.execute("SELECT state, COUNT(*) AS count FROM roamai_inbox GROUP BY state")
            }
            outbox = {
                row["state"]: row["count"]
                for row in connection.execute("SELECT state, COUNT(*) AS count FROM roamai_outbox GROUP BY state")
            }
