...
def search_flights(origin: str, destination: str, date: str, max_budget: Optional[float] = None) -> str:
    ...
    try:
        ...
        res = search_flights_handler({"departure_id": origin, "arrival_id": destination, "outbound_date": date, "currency": "SGD"})
        ...
        if max_budget:
            ...
            if filtered:
                ...
                normalized_flights = [
                    {"flight": f"Live Flight {origin}->{destination}", "airline": "Carrier", "price": float(max_budget), "date": date}
                ]
        ...
        try:
            ...
            flights_out = [
                {"flight": r.get("title", "Flight"), "airline": "Live Search", "price": float(max_budget or 120.0), "link": r.get("href")}
                for r in results
            ]
            return json.dumps(
                {
...
def search_hotels(destination: str, checkin_date: str, checkout_date: str, max_price_per_night: Optional[float] = None) -> str:
    ...
    """
    logger.info(
        f"🏨 [Live Tool Call] search_hotels(destination={destination}, checkin={checkin_date}, checkout={checkout_date}, max_price={max_price_per_night})"
    )
    try:
        from src.mcp.travelassistant.hotel_server import search_hotels_handler

        res = search_hotels_handler(
            {"location": destination, "check_in_date": checkin_date, "check_out_date": checkout_date, "currency": "SGD"}
        )
        ...
        if not props:
            ...
            props = [
                {"name": r.get("title", f"Stay in {destination}"), "description": r.get("body", ""), "link": r.get("href")} for r in results
            ]
...
def generate_itinerary(destination: str, days: int, preferences: str = "") -> str:
    ...
    try:
        ...
        for i in range(1, days + 1):
            ...
            schedule.append({"day": i, "theme": title, "activities": [snippet[:140]]})
        ...
        return json.dumps(
            {
                ...
                "schedule": schedule,
                "live_route_sources": [
                    {"title": r.get("title"), "snippet": r.get("body"), "link": r.get("href")} for r in results if isinstance(r, dict)
                ],
            },
            indent=2,
        )
        ...
        return json.dumps(
            {
                ...
                "days": days,
                "error": str(e),
                "schedule": [{"day": i, "theme": f"Day {i}"} for i in range(1, days + 1)],
            }
        )
...
def convert_currency(from_currency: str, to_currency: str, amount: float = 1.0) -> str:
    ...
    try:
        ...
        res = convert_currency_handler({"from_currency": from_currency, "to_currency": to_currency, "amount": amount})
