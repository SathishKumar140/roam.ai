...
class TopicFact(BaseModel):
    ...
    value: str = Field(
        min_length=1,
        max_length=300,
        description="Exact user wording for this field, contained in evidence_quote; do not normalize or infer",
    )
    evidence_message_id: str
...
class Observation(BaseModel):
    topic_id: str | None = Field(
        default=None, description="Copy an existing id from group_context.topics, or return null for a new topic. Never create an id."
    )
    comparison_topic_ids: list[str] = Field(
        default_factory=list,
        max_length=4,
        description="Existing topic IDs explicitly requested for a read-only comparison; otherwise empty",
    )
    ...
    continues_task: bool = Field(
        default=False,
        description="The latest speaker is clearly answering RoamAI's pending question in this existing active topic, not merely chatting or saying an ambiguous yes",
    )
    ...
    clarification_question: str = Field(
        default="",
        max_length=300,
        description="Ask a concise question when the target topic, field, unit, or requested change is ambiguous. No state may change until clarified.",
    )
