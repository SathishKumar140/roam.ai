...
class RoamAIService:
    ...
    def _callback(self, event):
        ...
        try:
            ...
            if len(parts) == 4 and parts[1] == "expense" and parts[3] in {"yes", "no"}:
                ...
                messages = {
                    "confirmed": "Everyone confirmed this expense; it is now active.",
            ...
            async with asyncio.timeout(140):
                if event.media and event.explicitly_addressed:
                    ...
                    if event.media.type in {"voice", "photo"} and adapter and hasattr(adapter, "media_bytes"):
                        try:
                            ...
                            if event.media.type == "voice":
                                ...
                                with self.store.connect() as connection:
                                    connection.execute(
                                        "UPDATE roamai_inbox SET payload = ? WHERE id = ? AND group_id = ?",
                                        (event.model_dump_json(), rows[-1]["id"], group_id),
                                    )
                            ...
                            self.store.complete(
                                rows,
                                response={
                                    "text": "I couldn't read that attachment. Please send a smaller image or describe it in text.",
                        ...
                        self.store.complete(
                            rows, response={"text": "I can't read attachments on this connection yet. Could you describe it in text?"}
                        )
                        return True
                    ...
                    if len(parts) == 3 and parts[1] in {"accept", "decline"} and parts[0] == "roamai":
                        ...
                        if not accepted or parts[1] == "decline":
                            self.store.complete(
                                rows,
                                response={
                                    "text": "No problem; I'll leave that with you."
                                    if accepted
                                    else "That offer is unavailable or was addressed to another member."
                                },
                            )
                            return True
                ...
                if comparing:
                    observation = observation.model_copy(
                        update={"topic_id": None, "facts": [], "preferences": [], "evidence_message_ids": sorted(batch_ids)}
                    )
                ...
                if forced_topic:
                    ...
                    referenced = [
                        item
                        for item in context["outbound"]
                        if event.reply_to_message_id in item["provider_ids"] and item["topic_id"] in known_topics
                    ]
                    if referenced:
                ...
                if observation.clarification_question and not forced_topic:
                    ...
                    if participation(observation, event, context, cooldown=self.cooldown) == "respond":
                        response = {
                            "text": observation.clarification_question,
                            "addressed_to": event.sender.id,
                            "reply_to_message_id": event.message_id,
                        }
                ...
                observation = observation.model_copy(
                    update={"facts": [fact for fact in observation.facts if fact.evidence_message_id in batch_ids]}
                )
                ...
                if decision == "decline":
                    ...
                    self.store.complete(
                        ...
                        topic_id,
                        {
                            "text": observation.offer,
                            "offered_to": source["sender_id"],
    ...
    def start(self):
        ...
        self.tasks = [
            asyncio.create_task(self._loop(operation))
            for operation in [self.process_once, self.process_once, self.deliver_once, self._timers]
        ]
    ...
    def readiness(self):
        ...
        configured = bool(
            ...
            and (settings.DEFAULT_LLM_PROVIDER == "gemini" or not settings.OPENAI_API_KEY)
            or settings.OPENAI_API_KEY
            and (settings.DEFAULT_LLM_PROVIDER == "openai" or not settings.GEMINI_API_KEY)
        )
        ...
        return {
            ...
            "checks": {
                ...
                "model_configured": configured and not fake_model,
                "queues_timely": database_ready and queues.get("overdue", 0) == 0,
            },
